...
## Hebrew Calendar Suite — Display app
- Lives in submodule `hebrew-calendar-suite/` (own remote: `wbdyhy0-create/hebrew-calendar-suite`).
- **Display PDF RTL**: `apps/display/src/utils/pdf.ts` mirrors the on-screen RTL day order (שבת on the visual left, ראשון on the right). A temporary stylesheet plus inline `setProperty('direction','rtl','important')` defeats the shared `.pdfMode .grid { direction: ltr !important }` rule from `packages/shared/src/utils/calendarDocumentStyles.ts`.
- **Display screen orientation**: `apps/display/src/App.tsx` forces the kiosk surface to A4 landscape (297×210 mm ≈ 1123×794 px @ 96dpi) regardless of `settings.pdfOrientation`. Matches the Studio 2026 wide layout. PDF orientation is independently forced to landscape in `downloadMonthPdf` / `downloadYearPdf`.
